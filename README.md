...
단, 3F는 사용자를 분류하기 위한 표가 아니라 "내 경험이 제대로 읽혔다"는 감각을 만든 뒤 자기 발견으로 이어지게 하는 코칭 렌즈다.
...
│   ├── chatgpt/
│   └── gemini/
├── prompts/                 # 짧은 명령어 + 유도 진입 + 코칭형 3F
├── templates/               # 데일리·주말·월간·코칭형 3F 템플릿
└── calendar/                # 캘린더 등록 문구
```
...
| `3F 정리` | 원문을 먼저 읽어준 뒤 Fact / Feeling / Finding으로 정리 + 분기 질문 |
| `코칭형 3F` | 불릿 분류 대신 읽힘 → 상황 재구성 → 감정 구조 → 발견 → 다음 질문으로 출력 |
